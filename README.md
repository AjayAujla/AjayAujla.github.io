...
1. Create a React app named `stockMarketHelper`:

    > In case you want to use a different name from `stockMarketHelper` (e.g. `web-ui`), you can accomplish that by replacing all occurrences of `stockMarketHelper` in this tutorial, with that other name (i.e. `stockMarketHelper` --> `web-ui`).
  
    ```shell
    $ npx create-react-app stockMarketHelper
    ```
    ...
    > ```shell
    > $ npx create-react-app stockMarketHelper --template typescript
    > ```

    That command will create a new folder named `stockMarketHelper`, which will contain the source code of a React app.
...
2. Add a `homepage` property in this format\*: `https://{username}.github.io/{repo-name}`
    ...
    ```diff
    {
      "name": "stockMarketHelper",
      "version": "0.1.0",
    + "homepage": "https://ajayaujla.github.io/react-gh-pages",
      "private": true,
    ```
...
1. Add a "[remote](https://git-scm.com/docs/git-remote)" to the local Git repository.
    ...
    ```shell
    $ git remote add origin https://github.com/ajayaujla/react-gh-pages.git
    ```
...
1. Deploy the React app to GitHub Pages
    ...
    > By default, the new commit on the `gh-pages` branch will have a commit message of "Updates". You can [specify a custom commit message](https://github.com/ajayaujla/react-gh-pages/issues/80#issuecomment-1042449820) via the `-m` option, like this:
    > ```shell
...
At this point, the React app is accessible to anyone who visits the `homepage` URL you specified in Step 4. For example, the React app I deployed is accessible at https://ajayaujla.github.io/react-gh-pages.
...
Note: I specified the avatars using HTML because, when I did so using Markdown,
      ...
      (e.g. 40px squared, for `https://github.com/ajayaujla.png?size=40`);
...
<a href="https://github.com/ajayaujla" target="_blank" title="ajayaujla">
  <img src="https://github.com/ajayaujla.png?size=40" height="40" width="40" alt="ajayaujla" />
</a>
